...
export const handleAppUsageAggregatePost = async (request: Request, env: Env): Promise<Response> => {
  try {
    ...
    const targetDate = typeof body?.target_date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(body.target_date)
      ? body.target_date
      : getPreviousJstDateFrom(Date.now(), dayStartHour);
    const aggregate = await aggregateStudyUsageForTargetDate(env, targetDate);
    return jsonResponse(200, { ok: true, target_date: targetDate, day_start_hour: dayStartHour, daily_log_updated: true, aggregate: { study_minutes: aggregate.minutes, study_sessions: aggregate.sessions, study_last_used_at: aggregate.last } });
  } catch (error) {
