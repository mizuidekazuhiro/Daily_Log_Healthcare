...
| 新規 | `Readiness Label` | select | `readiness_label` | 任意 |

> `Sleep Source` や `Readiness Label` は `autosleep` / `healthkit` / `OK` など、使いたい候補を Notion 側で追加しておくと運用しやすいです。

### Notion 列追加手順

1. 既存の Daily Log Database を開く
2. 上の「推奨Notion列一覧」に従って不足列を追加する
3. Number 型の列は数値、Date 型の列は日付、Select 型の列は候補を追加する
...
| `readiness_label` | `Readiness Label` | select | 任意 |
...
- Notion 側では `REM Duration Min` から `Readiness Label` までの新規列を追加してください。
